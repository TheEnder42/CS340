...
(defn tally-item [prices line-item]
  (* ((first line-item) prices) (second line-item)))
...
;;
(defn mulv3 [vec fac]
  (let [first-bit (* fac (first vec))
        second-bit (* fac (first(rest vec)))
        third-bit (* fac (first (rest (rest vec))))]
       (vector first-bit second-bit third-bit))) ;java.lang.Double cannot be cast to clojure.lang.IFn
...
;;
(defn mulv [vec fac]
  (mapv (fn [x] (* fac x)) vec)) ;Unable to resolve symbol: % in this context
